...
# V1: imgs and seg mask are in two different conv blocks
@persistence.persistent_class
...
# Tri-discriminator: upsampled image, super-resolved image, and segmentation mask
@persistence.persistent_class
class HoloDiscriminator(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
        seg_resolution,                 # Input resolution.
        seg_channels,                   # Number of input color channels.
        parsing_resolution,                 # Input resolution.
        parsing_channels,                   # Number of input color channels.
        architecture        = 'resnet', # Architecture: 'orig', 'skip', 'resnet'.
        channel_base        = 32768,    # Overall multiplier for the number of channels.
        channel_max         = 512,      # Maximum number of channels in any layer.
        num_fp16_res        = 4,        # Use FP16 for the N highest resolutions.
        ...
        super().__init__()
        img_channels = img_channels * 2 + seg_channels + parsing_channels
        ...
        self.seg_resolution = seg_resolution
        self.seg_channels = seg_channels
        self.parsing_resolution = parsing_resolution
        self.parsing_channels = parsing_channels
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs): # img: (bs, 7, 512, 512), c: (bs, 25)
        image_raw = filtered_resizing(img['image_raw'], size=img['image'].shape[-1], f=self.resample_filter)
        seg = filtered_resizing(img['image_mask'], size=img['image'].shape[-1], f=self.resample_filter)
        seg = 2 * seg - 1 # normalize to [-1,1]
        parsing = filtered_resizing(img['image_parsing'], size=img['image'].shape[-1], f=self.resample_filter)
        parsing = 2 * parsing - 1 # normalize to [-1,1]
        img = torch.cat([img['image'], image_raw, seg, parsing], 1)
        ...
        x = self.b4(x, img, cmap) # (bs, 1); (bs,512,4,4), None, (bs, 512)
        return x
    ...
    def extra_repr(self):
        return ' '.join([
            ...
            f'img_resolution={self.img_resolution:d}, img_channels={self.img_channels:d},',
            f'seg_resolution={self.seg_resolution:d}, seg_channels={self.seg_channels:d}',
            f'parsing_resolution={self.parsing_resolution:d}, parsing_channels={self.parsing_channels:d}'])    
