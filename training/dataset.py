...
class CameraLabeledDataset(ImageFolderDataset):
    ...
    def _filter_samples(self):
        ...
        back_repeat_idx = np.tile(back_idx, self.back_repeat)
        # merge front index and repeated back index
        new_idx = np.concatenate((front_idx, back_repeat_idx))
        print(f"Repeat {len(back_idx)} back images till abs({self.max_yaw}) degree {self.back_repeat} times")
        return new_idx
...
class MaskLabeledDataset(ImageFolderDataset):
    ...
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        self.max_pitch = 90 if max_pitch is None else max_pitch
        self.back_repeat = 1 if back_repeat is None else back_repeat
        super().__init__(path=img_path, **super_kwargs)
...
class ImageFolderDataset_PTI(Dataset):
    def __init__(self,
        path,                   # Path to directory or zip.
        resolution      = None, # Ensure specific resolution, None = highest available.
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        name = os.path.splitext(os.path.basename(self._path))[0]
        raw_shape = [len(self._image_fnames)] + list(self._load_raw_image(0,resolution=resolution).shape)
        if resolution is not None and (raw_shape[2] != resolution or raw_shape[3] != resolution):
            raise IOError('Image files do not match the specified resolution')
        super().__init__(name=name, raw_shape=raw_shape, **super_kwargs)
    ...
    def _load_raw_image(self, raw_idx, resolution=None, resize_mode='bilinear'):
        fname = self._image_fnames[raw_idx]
        with self._open_file(fname) as f:
            image = PIL.Image.open(f)
            if resolution is not None:
                if resize_mode == 'bilinear':
                    image = image.resize((resolution, resolution), PIL.Image.BILINEAR)
                elif resize_mode == 'nearest':
                    image = image.resize((resolution, resolution), PIL.Image.NEAREST)
                else:
                    raise
