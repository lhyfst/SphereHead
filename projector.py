...
def project(
    ...
    *,
    ...
    # vgg16 = torch.load('/home/chence/.cache/torch/hub/checkpoints/vgg16-397923af.pth').eval().to(device)
...
@click.option('--filename',       help='Target image file to project to', required=True, metavar='FILE')
...
def run_projection(
    ...
    target_fname = os.path.join(target_fname, filename)
    outdir = os.path.join(outdir, os.path.basename(network_pkl), filename)
    ...
    if target_fname is not None:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset_PTI', path=target_fname, use_labels=True, max_size=None, xflip=False)
